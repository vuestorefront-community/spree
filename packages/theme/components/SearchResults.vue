<template>
  <div>
    <SfMegaMenu
      ...
      :title="$t('components.search_results.search_results')"
      class="search"
    >
      <transition name="sf-fade" mode="out-in">
        <div v-if="products && products.length > 0" class="search__wrapper-results" key="results">
          <SfMegaMenuColumn :title="$t('components.search_results.categories')" class="sf-mega-menu-column--pined-content-on-mobile search__categories">
          ...
          <SfMegaMenuColumn :title="$t('components.search_results.product_suggestions')" class="sf-mega-menu-column--pined-content-on-mobile search__results">
            <template #title="{title}">
          ...
          <div class="action-buttons smartphone-only">
            <SfButton class="action-buttons__button color-light" @click="$emit('close')">{{ $t('components.search_results.cancel') }}</SfButton>
          </div>
        </div>
        <div v-else key="no-results" class="before-results">
          ...
          <template v-else>
            <p class="before-results__paragraph">{{ $t('components.search_results.you_havent_searched_for_items_yet') }}</p>
            <p class="before-results__paragraph">{{ $t('components.search_results.lets_start_now_we_will_help_you') }}</p>
          </template>
          <SfButton class="before-results__button color-secondary smartphone-only" @click="$emit('close')">{{ $t('components.search_results.go_back') }}</SfButton>
        </div>
      </transition>
    </SfMegaMenu>
  </div>
...
import { useWishlist, wishlistGetters, productGetters } from '@vue-storefront/spree';
