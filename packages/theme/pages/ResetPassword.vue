<template>
  ...
  >
    <template #modal-bar>
      <SfBar
        class="sf-modal__bar"
        :title="$t('pages.reset_password.reset_password')"
      />
    </template>
          ...
          <form class="form" @submit.prevent="handleSubmit(setNewPassword)">
            ...
            >
              <SfLoader :class="{ loader: forgotPasswordLoading }" :loading="forgotPasswordLoading">
                <div>{{ $t('pages.reset_password.save_password') }}</div>
              </SfLoader>
            </SfButton>
          </form>
        </ValidationObserver>
      ...
      <div v-else>
        <p>{{ $t('pages.reset_password.password_changed') }}</p>
        <SfButton class="sf-button--text" link="/">
          {{ $t('pages.reset_password.back_to_home') }}
        </SfButton>
      </div>
  </SfModal>
</template>
